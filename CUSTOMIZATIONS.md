...
- `docker/php-fpm-entrypoint`: a `migrate` role. Any command containing `artisan migrate` dispatches to it, runs `apply_php_configuration` and `wait_for_db`, and then execs the command. It skips `prep_file_permissions` and `prep_storage`, and does not run telemetry initialization, Passport key generation, or `optimize`, so an explicit migration run does not also execute the API startup path. The private deployment repository uses this as a deploy step that applies schema changes before application containers are replaced. After upstream updates, re-check that the role dispatch in `main()` still matches `artisan migrate` before falling through to the `api` default, and that the `migrate` branch still skips the storage and permission preparation.
- `docker/php-fpm-entrypoint`: the `api` role's `apply_db_migrations` call is gated on `OPNFORM_RUN_MIGRATIONS`, which defaults to `true`. Unset or `true` keeps the upstream behavior, so `docker-compose.yml`, `docker-compose.dev.yml`, `docker-compose.e2e.yml`, `docker-compose.codex.yml`, and anyone running the upstream image are unaffected. Staging sets it to `false` so container readiness is not gated on schema work. After upstream updates, re-check that upstream has not moved the migration call out of `apply_db_migrations`, or out of the `api` branch, which would bypass the flag.
- `openspec/`: this fork's change planning, holding proposals, specs, designs, and task lists for OpenSpec-tracked work. Initialized with `--tools none`, so no AI tool file was generated and the upstream-owned `AGENTS.md` is not modified. Upstream owns no path under this directory, so it needs no attention during an upstream merge.
