...
- `api/app/Service/Branding/BrandingPolicy.php`: `canRemoveBranding()` returns `true` unconditionally when `config('app.self_hosted')`, before the paid `Feature::BRANDING_REMOVAL`/license check. This removes the "PDF generated with OpnForm" footer from generated PDFs regardless of the per-template `remove_branding` toggle, matching the unconditional form-page badge removal above. It also makes `canRemoveFormBranding()` (used by `FormEmailNotification`) return `true` unconditionally, so notification emails never show OpnForm branding either. Cloud/hosted behavior (non self-hosted) is unchanged — still gated by the workspace's `branding.removal` feature. After upstream updates, re-check that `PdfGeneratorService::generatePdfContent()` still calls `BrandingPolicy::canRemoveBranding()` to decide `$addBranding`, and generate a PDF on a self-hosted instance to confirm no footer is added.
